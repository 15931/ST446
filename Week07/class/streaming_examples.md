...
Now, we look at another streaming example.
We stream Twitter data (live) and read tweets that involve `Trump`.
...
Run the [`kafka_twitter_producer.py`](kafka_twitter_producer.py) in `~` to get tweets from the twitter API
   ...
   * Upload `kafka_twitter_producer.py` to your master node (though a bucket) and update it with your own twitter information (for eample using the nano editor). See [access-tokens](https://developer.twitter.com/en/docs/basics/authentication/guides/access-tokens) for more details.
   * You might need to apply for a [twitter developer account and create a twitter app](https://developer.twitter.com/en/apps/create) to access the required tokens and keys
   * You will need to add your own `consumer_key`, `consumer_secret`, `access_token` and `access_token_secret` generated by twitter to the python file. They all *should* look like gibberish.
...
### Importing the Kafka twitter stream into PySpark streaming (homwork)
Instead of using a python script to consume messages, we can also pass them to pyspark as before.

Have a look at [kafka_twitter_pyspark.py](kafka_twitter_pyspark.py) and upload it to the cluster root path.

Download the NLTK data using the python shell
...
True
```
and then copy them to `/home` directory:
```
jason_yijialin@jialin-cluster-m:~$ sudo cp -r nltk_data/ /home/
```

Then `cd` to `$SPARK_HOME` run it like so:
...
Which produced a stream of ouput with content like this:
